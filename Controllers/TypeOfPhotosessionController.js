...
const GetTypesOfPhotography = async (req, res) => {
  try {
    ...
    if (response) {
      ...
      res
        .status(200)
        .send({ success: true, msg: "No photo session types found" });
    }
  } catch (error) {
...
const GetTypeOfPhotographyByTypeName = async (req, res) => {
  try {
    ...
    const response = await TypesOfPhotoshoot.findOne({
      typeOfPhotography: name,
    });
    ...
    if (response) {
      ...
      res
        .status(200)
        .send({ success: true, msg: "No photo session found by name" });
    }
  } catch (error) {
    res.status(404).send({ success: false, msg: error });
  }
...
const CreateTypeOfPhotography = async (req, res) => {
  try {
    ...
    if (existingType) {
      return res
        .status(404)
        .json({ message: "Type already exists", success: false });
    }
...
const DeleteTypeOfPhotographyById = async (req, res) => {
  try {
    ...
    if (deletedRes.deletedCount === 1) {
      ...
      res
        .status(200)
        .send({ success: false, msg: "No type of photo session found" });
    }
  } catch (error) {
    res.status(404).send({ success: false, msg: error });
  }
